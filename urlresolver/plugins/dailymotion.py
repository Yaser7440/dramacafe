...
class DailymotionResolver(UrlResolver):
    ...
    def get_media_url(self, host, media_id):
        web_url =common.web_url()# self.get_url(host, media_id)
        if "plugin:" in web_url:
            web_url='http://www.dailymotion.com/video/'+web_url.split("url=")[1]
       ...
       # print "html",html
        ...
        videoUrl = []
        try: videoUrl.append(('1080',html['1080'][0]['url']))
        except: pass
        try: videoUrl.append(('720',html['720'][0]['url']))
        except: pass
        try: videoUrl.append(('480',html['480'][0]['url']))
        except: pass
        try: videoUrl.append(('380',html['380'][0]['url']))
        except: pass
        try: videoUrl.append(('240',html['240'][0]['url']))
        except: pass
        try: videoUrl.append(('auto',html['auto'][0]['url']))
        except: pass
        print "videoUrl",videoUrl
        for item in videoUrl:
            common.addDir(item[0],item[1],1,'',1)
        return html['720'][0]['url']
        vUrl = ''
